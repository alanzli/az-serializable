...
public:
    ...
    const std::vector<SerializationError>& getErrors() const {
        return errors_;
    }
...
public:
    ...
    // Get error collector
    const SerializationErrorCollector& getErrorCollector() const {
        return error_collector_;
    }
    
    bool hasErrors() const {
        return error_collector_.hasErrors();
    }
...
protected:
    ...
    virtual void serializeObject(const std::string& name, const std::vector<std::pair<std::string, std::string>>& json_pairs) override = 0;
    ...
    virtual std::string createJsonObject(const std::vector<std::pair<std::string, std::string>>& pairs) const override = 0;
