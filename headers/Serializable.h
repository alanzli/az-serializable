...
public:
    ...
    // Fixed-width integer types (only when different from standard types)
    ...
    // Conditionally add int32_t and int64_t only if they're different from standard types
    ...
    // Conditionally add uint32_t and uint64_t only if they're different from standard types
    ...
    // Template dispatch method - public so it can be called by user code
    template<typename T>
    void serializeProperty(const std::string& name, const T& value) {
        if constexpr (std::is_same_v<T, int>) {
            ...
            // Use the appropriate method based on what int32_t actually is
            ...
            // Use the appropriate method based on what int64_t actually is
            ...
            // Use the appropriate method based on what uint32_t actually is
            ...
            // Use the appropriate method based on what uint64_t actually is
    ...
    // For nested serializable objects - public so it can be called directly
...
private:
    // Helper methods for container serialization
    ...
    std::string serializeElementToString(const T& element) {
        if constexpr (std::is_same_v<T, int>) {
            ...
            // Create a temporary serializer for nested objects
...
protected:
    ...
    virtual std::string escapeJsonString(const std::string& input) const {
        // Default implementation - subclasses should override for proper escaping
        return input;
    }
...
public:
    ...
    void serialize(TypedSerializer& serializer) const {
        visitProperties(serializer);
    }

protected:
    // Child classes implement this to visit their properties
    virtual void visitProperties(TypedSerializer& serializer) const = 0;
};
